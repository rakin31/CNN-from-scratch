...
class maxPool:
    ...
    def backward(self, doutput, learning_rate):
        ...
        d_a = doutput.repeat(height_poolwindow, axis=2).repeat(width_poolwindow, axis=3)
        # print("dA.shape", dA.shape)
        d_a = np.multiply(d_a, mask)
        # print(dA)
        pad = np.zeros(input_tensor.shape)
        # print("pad", pad)
        pad[:, :, :d_a.shape[2], :d_a.shape[3]] = d_a
...
class FullyConnected_Layer:
    ...
    def forward(self, input_tensor):
        ...
        x = np.dot(self.weights, input_tensor.T)
        x = x + self.bias
        rtn_val = x.T
        return rtn_val
...
class LeNetModel:
    ...
    def reset_params_all(self):
        self.conv1.reset_params()
        self.relu1.reset_params()
        self.maxpool1.reset_params()
        self.conv2.reset_params()
        self.relu2.reset_params()
        self.maxpool2.reset_params()
        self.flatten.reset_params()
        self.fc1.reset_params()
        self.relu3.reset_params()
        self.fc2.reset_params()
        self.relu4.reset_params()
        self.fc3.reset_params()
        self.softmax.reset_params()
...
def train(y_info):
    ...
    img_paths = []
    batch_no = 0
    my_model = LeNetModel(28)
    x_train_len = int(len(y_info) * 0.8)
    for epoch in range(total_epochs):
        print("epoch no: ", epoch + 1)
        batch_no = 0
        train_loss = 0
        for i in range(0, x_train_len, batch_size+1):
            batch_no += 1
            start = i
            end = i + (batch_size - 1)
            if end >= len(y_info):
                end = len(y_info) - 1
            x_train, y_train = read_images_create_matrix(y_info, start, end)
            if len(x_train) == 0:
                break
            ...
            loss = (y_prediction - ohl)/batch_size
            t_loss = find_cross_entropy_loss(y_prediction, ohl)
            train_loss += t_loss
            back = my_model.backward(loss, 0.001)


        train_loss = train_loss/len(y_info)
        validation_size = int(len(y_info) * 0.2)
        x_train, y_train = read_images_create_matrix(y_info, x_train_len, len(y_info))
        y_real = y_train
        if len(x_train) == 0:
            continue
        mean = np.mean(x_train)
        ...
        my_model.reset_params_all()
        with open('1705026_model.pickle', 'wb') as f:
            pkl.dump(my_model, f)
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        folder_name = './training-a/'
        df = pd.read_csv('training-a.csv')
        y_train_digit = df['digit'].values
        y_train_filename = df['filename'].values
        y_info = []
        for i in range(len(y_train_filename)):
            img_path = os.path.join(folder_name, y_train_filename[i])
            y_info.append((img_path, y_train_digit[i]))
        # print
        my_model = train(y_info)
        print()
        print()
        print("=========================================")
        print("Training done\n\n")
